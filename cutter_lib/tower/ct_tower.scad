...
module ct_tower_example(width, height, depth, wall_thickness,
...
module ct_tower_arrange(width, height, depth, wall_thickness,
                make_3d=false, spacing_2d=1)
{
    ...
    else
    {
        size = get_ct_tower_floor_2d_size(width=width, height=height, 
                                          depth=depth, wall_thickness=wall_thickness);
        for (i = [0:no_floors-1])
        {
